...
/// The largest width whose `2^width` ordinals fit a `u128` cursor.
...
pub fn bit_vectors(width: usize) -> Result<BitVectorIter, CombError> {
    ...
    let total = if width == MAX_WIDTH {
        u128::MAX // 2^127 - 1 cursor range; the final vector is emitted on wrap
    } else {
        1u128 << width
    };
    Ok(BitVectorIter {
...
pub fn bit_vector_unrank(rank: &BigUint, width: usize) -> Vec<bool> {
    (0..width).map(|j| rank.bit(j as u64)).collect()
}
...
mod tests {
    ...
    fn rank_unrank_round_trip() {
        for (i, bits) in bit_vectors(5).unwrap().enumerate() {
            ...
            assert_eq!(bit_vector_unrank(&r, 5), bits);
        }
    }
